...
public class PathFinder extends Application {
    ...
    public void start(Stage stage) {
        ...
        buttons.getChildren().addAll(findPathButton, showConnectionButton, newPlaceButton, newConnectionButton,
        ...
        try {
            ...
            for (Place key : graph.getNodes()) {
                for (Edge<Place> edge : graph.getEdgesFrom(key)) {
                    writer.println(key.getName() + ";" + ((Place) edge.getDestination()).getName() + ";"
    ...
    public void showConnection() {
        if (!isConnected(selected[0], selected[1]))
            return;
    ...
    private void findPath() {
        ...
        for (Edge<Place> edge : path) {
            textArea.appendText("to " + edge.getDestination().getName() + " by " + edge.getName() + " takes "
                    + edge.getWeight() + "\n");
            time += edge.getWeight();
        }
